Shader "SH_AW_PBR_ORM" // Especially for the store "Abandoned World" v. 1.1
{
    Properties
    {
        _ColorMul ("Color", Color) = (1,1,1,1)
        _base_color ("Albedo (RGB)", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            fixed4 c = tex2D (_base_color, IN.uv_base_color) * _ColorMul;
            o.Albedo = c.rgb;
			o.Normal = UnpackNormal(tex2D(_NormalMap, IN.uv_NormalMap)) * float3(1, _NormalStrength, 1);
